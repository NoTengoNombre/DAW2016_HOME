...
function peticionPing() {
	...
	temporizadorPing = setTimeout(function() { interrumpirPing(oXML) }, timeoutPing + 1000);	// Por si el timeout no funciona bien
	r0 = Date.now();
	oXML.send(null);
}
		...
		if (oXML.readyState == 4) {
			if (oXML.status == 200) {
				...
				if (t1 - t0 < duracionBajada) {
					setTimeout(function() { peticionBajada(idPeticion) }, 0);
				}
			}
		}
	} else {
		...
		if (oXML.status == 200 || oXML.status == 0) {
			...
			if (!finU) {
				...
				if (u1 - u0 < duracionSubida) {
					setTimeout(function() { peticionSubida(cont) }, 0);
				}
			}
		}
	}
...
function ponGrados(grados, velo) {
	...
	if (gradosIndicados != grados && velo == 0) {
		setTimeout(function() { ponVelo(velo) }, 100);
	}
}
